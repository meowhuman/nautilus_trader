...
### 3. Settings.json 配置
hooks 透過 `.claude/settings.json` 配置，使用**絕對路徑**確保可靠性：
```json
"hooks": {
  "Stop": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "python /absolute/path/to/.claude/hooks/stop.py --chat"
        }
      ]
    }
  ]
...
**重要**：必須用絕對路徑！相對路徑 `uv run .claude/hooks/...` 會因工作目錄唔同而失敗。
...
### 常見錯誤
- `No such file or directory` → **使用絕對路徑**，唔好用相對路徑。相對路徑因工作目錄改變而失敗
...
1. **絕對路徑優先**：settings.json 中使用絕對路徑，避免工作目錄變化導致失敗
2. **統一 shebang**：所有 hooks 使用 `#!/usr/bin/env uv run python`
3. **錯誤處理**：hooks 內應該包含適當嘅 try-catch
4. **日誌記錄**：重要 hooks 應該記錄到 `logs/` 目錄
5. **性能考慮**：hooks 執行要快，避免阻塞主程序
