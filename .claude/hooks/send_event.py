...
def save_event_locally(event_data, log_dir):
    """Save event data to local logs directory."""
    try:
        # Ensure directory exists
        log_dir.mkdir(parents=True, exist_ok=True)

        # Create events.jsonl file (append-only log)
...
def send_event_to_server(event_data, server_url='http://localhost:4000/events'):
    """Try to send to observability server, fallback to local storage."""
    try:
        # Prepare the request
        ...
        print(f"Server unavailable, saving locally instead", file=sys.stderr)
        return False
...
def main():
    ...
    # If server unavailable, save locally
    if not success:
        log_dir = Path(input_data.get('session_id', 'unknown'))
        save_event_locally(event_data, log_dir)
